...
cdef class Sampler:
    ...
    def topy(self):
        ...
        sample_probs = defaultdict(list)
        sample_set0 = defaultdict(list)
        degree = []
        adj = defaultdict(list)

        for it in self.sample_weights:
            for v in it.second:
                sample_weights[it.first].append(v)
        for it in self.sample_probs:
            for v in it.second:
                sample_probs[it.first].append(v)
        for it3 in self.sample_set0:
            for i in it3.second:
                sample_set0[it3.first].append(i)
        for i in self.degree:
            degree.append(i)
        for it3 in self.adj:
            for i in it3.second:
                adj[it3.first].append(i)

        return self.num_proc, self.neighbor_limit, self.max_reward, self.num_node, \
                sample_weights, sample_probs, sample_set0, degree, adj

    def frompy(self, num_proc, neighbor_limit, max_reward, num_node, sample_weights, sample_probs,
               sample_set0, degree, adj):
        self.num_proc = num_proc
        self.neighbor_limit = neighbor_limit
        self.max_reward = max_reward
        self.num_node = num_node
        self.sample_weights.clear()
        self.sample_probs.clear()
        self.sample_set0.clear()
        self.degree.clear()
        self.adj.clear()

        for k, vec in sample_weights.items():
            for v in vec:
                self.sample_weights[k].push_back(v)
        for k, vec in sample_probs.items():
            for v in vec:
                self.sample_probs[k].push_back(v)
        for k, vec in sample_set0.items():
            for v in vec:
                self.sample_set0[k].push_back(v)
        for v in degree:
            self.degree.push_back(v)
        for k, vec in adj.items():
            for v in vec:
                self.adj[k].push_back(v)
    ...
    def reset_parallel(self):
        ...
        with nogil, parallel(num_threads=self.num_proc):
            for p in prange(self.num_proc, schedule='dynamic'):
                self._reset_parallel(p)

    cdef void _reset_parallel(self, int p) nogil:
        cdef int src = 0
        cdef int degree
        cdef int idx
        while src < self.num_node:
            if src % self.num_proc != p:
                src += 1
                continue
    ...
    def reset_weight_probs(self):
        for src in range(self.num_node):
            if self.degree[src] == 0:
                continue
            self._reset_w_p(src)

    cdef void _reset_w_p(self, int src):
        cdef int degree, idx
        degree = self.degree[src]
        if degree == 0:
            return
...
cdef class BanditSampler(Sampler):
    ...
    cdef void update_sample_weights(
        ...
        while i < num_data:
            ...
            reward = delta*att_val/self.sample_probs[src][idx]**2
    ...
    cdef int sample_neighbors_v1(self, int node, vector[int]& edges,
        ...
        degree = self.degree[node]
        edge_size = edges.size()
        cdef vector[int].iterator it # why use this iterator?
        cdef int i
...
cdef class ThanosExp3(Sampler):
    ...
    cdef void update_sample_weights(
        ...
        while i < num_data:
            ...
            att_val = att_list[att_map[src][dst]]
            # reward = delta*att_val**2/self.sample_probs[src][idx]**2
            reward = delta*att_val
            if self.max_reward > 0 and reward > self.max_reward:
                reward = self.max_reward
            elif reward < 0:
                reward = 0
            self.sample_weights[src][idx] *= exp(reward)
            # self.sample_weights[src][idx] *= exp(reward/self.sample_probs[src][idx])
            i += 1
...
cdef class BanditLinearSampler(Sampler):
    ...
    cdef void update_sample_weights(
        ...
        while i < num_data:
            ...
            reward = delta*att/self.sample_probs[src][idx]**2
            if reward > self.max_reward:
                reward = self.max_reward
...
cdef class BanditMPSampler(Sampler):
    ...
    def update(self, np.ndarray[int,ndim=1,mode='c'] np_src_list,
        ...
        cdef float eta = FLAGS.eta
        cdef float max_reward = FLAGS.max_reward
        with nogil, parallel(num_threads=self.num_proc):
        # with nogil:
            for p in prange(self.num_proc, schedule='dynamic'):
    ...
    cdef void update_sample_weights(
        ...
        while src_idx < num_src:
            ...
            while i < src_end[src]:
                ...
                # att_val = att_list[att_idx]
                reward = delta*att_list[att_idx]
                if self.max_reward > 0 and reward > self.max_reward:
                    reward = self.max_reward
                elif reward < 0:
                    reward = 0
