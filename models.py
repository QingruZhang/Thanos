...
def attention_mechanism(name, v, W_s, W_d, V, cur_embed, left, right, n2n):
    ...
    # a_{i,j} \propto v^\top tanh (W_s \mu_i + W_d \mu_j)
        ...
        # t = tf.ones_like(tf.matmul(t, tf.reshape(v, [-1,1])))
        t = tf.matmul(t, tf.reshape(v, [-1,1]))
        sparse_attention = tf.SparseTensor(n2n.indices, tf.reshape(t, [-1]), n2n.dense_shape)
        sparse_attention = tf.sparse_softmax(sparse_attention)
    elif name == 'generalized_linear2':
        tl = tf.matmul(cur_embed, W_s)
        tl = tf.sparse_tensor_dense_matmul(sp_a=left, b=tl) # m by k
        tr = tf.matmul(cur_embed, W_d)
        tr = tf.sparse_tensor_dense_matmul(sp_a=right, b=tr)
        t = tf.nn.tanh(tf.add(tl,tr))
        # t = tf.ones_like(tf.matmul(t, tf.reshape(v, [-1,1])))
        t = tf.matmul(t, tf.reshape(v, [-1,1]))
        sparse_attention = tf.SparseTensor(n2n.indices, tf.reshape(t, [-1]), n2n.dense_shape)
        ...
        sparse_attention = tf.SparseTensor(n2n.indices, n2n.values, n2n.dense_shape)
    else:
        sys.exit(-1)
    return sparse_attention

def evaluate_reward(cur_embed, n2npool, sparse_attention, left, right):
    Hr = tf.sparse_tensor_dense_matmul(sp_a=right, b=cur_embed)
    Hbar = tf.sparse_tensor_dense_matmul(sp_a=left, b=n2npool)
    Degree = tf.sparse.reduce_sum(sparse_attention, axis=1)
    Degree = tf.sparse_tensor_dense_matmul(sp_a=left, b=tf.reshape(Degree, [-1,1]))
    Hr_norm =  tf.reduce_sum(tf.multiply(Hr, Hr), axis=1)
    inner_prod = tf.divide(tf.reduce_sum(tf.multiply(Hr, Hbar), axis=1), tf.reshape(Degree, [-1]))
    Reward = tf.subtract(2*inner_prod , Hr_norm)
    return Reward

def evaluate_reward_EdgeWeight(cur_embed, n2npool, sparse_attention, left, right):
    Hr = tf.sparse_tensor_dense_matmul(sp_a=right, b=cur_embed)
    Hr = tf.multiply( tf.reshape(sparse_attention.values, [-1,1]), Hr)
...
def evaluate_reward_realBS(cur_embed, n2npool, sparse_attention, left, right):
    Hr = tf.sparse_tensor_dense_matmul(sp_a=right, b=cur_embed)
    Hr_norm =  tf.reduce_sum(tf.multiply(Hr, Hr), axis=1)
    # Hr_norm = tf.norm(Hr, ord='euclidean', axis=1)
    Reward = tf.multiply(sparse_attention.values, Hr_norm)
    Reward = tf.multiply(sparse_attention.values, Reward)
    return Reward
    ...
    def _build(self):
        ...
        for i in range(2):
            ...
            sparse_attention = attention_mechanism(
                    "generalized_linear2", self.vars_v[i], self.vars_ws[i], self.vars_wd[i],
            ...
            if i == 0:
                self.cosine_reward = evaluate_reward(cur_embed, n2npool, sparse_attention, self.left, self.right)
                self.cosine_reward_addedge = evaluate_reward_EdgeWeight(cur_embed, n2npool, sparse_attention, self.left, self.right)
                self.real_bs_reward = evaluate_reward_realBS(cur_embed, n2npool, sparse_attention, self.left, self.right)
                self.n2npool_l0_selected_nodes = tf.sparse_tensor_dense_matmul(sp_a=self.node_select, b=n2npool)

            n2npool_addself = tf.add(n2npool, cur_embed) if not FLAGS.add_selfloop else n2npool
            node_linear = tf.matmul(n2npool_addself, self.vars_wn[i])+self.vars_bn[i]
    ...
    def _build(self):
        ...
        for i in range(2):
            if i == 0:
                ...
                self.vars_ws.append(glorot([hidden_dim, hidden_dim], 'W_s_%d'%i))
                self.vars_wd.append(glorot([hidden_dim, hidden_dim], 'W_d_%d'%i))
                self.vars_wt.append(glorot([hidden_dim, hidden_dim], 'W_t_%d'%i))
                self.vars_v.append(glorot([hidden_dim], 'v_%d'%i))
                self.vars_V.append(glorot([hidden_dim, hidden_dim], 'V_%d'%i))
            else:
                self.vars_wn.append(glorot([hidden_dim, hidden_dim], 'W_n_%d'%i))
        ...
        for i in range(bp):
            ...
            if FLAGS.residual == 1:
                merged_linear = tf.add(node_linear, node_embed)
            elif FLAGS.residual == 2:
                merged_linear = tf.add(node_linear, tf.matmul(cur_embed, self.vars_wt[i]))
            else:
                merged_linear = node_linear
...
class GCN(Model):
    ...
    def _build(self):
        ...
        for i in range(bp):
            ...
            # build sparse attention matrix a_{i,j}
            sparse_attention = attention_mechanism(
                    "gcn", self.vars_v[i], self.vars_ws[i], self.vars_wd[i],
                    self.vars_V[i], cur_embed, self.left, self.right, self.n2n)
            if i == 0:
