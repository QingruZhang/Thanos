...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'cSBM', 'Dataset string.')
flags.DEFINE_string('rootlogdir', '/efs/GNNBanditLog/log', 'rootdir for log folder')
...
# n=flag.cSBM_n, d=cSBM_d, Lambda=flag.cSBM_lam, p=flag.cSBM_p, mu=flag.cSBM_mu, sigma=flag.cSBM_sigma
flags.DEFINE_integer('cSBM_n', 1000, 'Number of Nodes')
flags.DEFINE_integer('cSBM_d', 10, 'Ave Degree')
flags.DEFINE_integer('cSBM_p', 200, 'Feature Dim')
flags.DEFINE_float('cSBM_lam', 1., 'Ration of Intra/Inter edges')
flags.DEFINE_float('cSBM_mu', 1., 'Ration of Intra/Inter edges')
flags.DEFINE_float('cSBM_sigma', 1., 'Ration of Intra/Inter edges')
...
def get_logger_addr(rootdir='/efs/GNNBanditLog/log'):
    ...
    if FLAGS.dataset == 'cSBM':
        dataset = "cSBM-n%d-d%d-Lam%.2f-p%d-mu%.2f-sigma%.2f"%(
                FLAGS.cSBM_n, FLAGS.cSBM_d, FLAGS.cSBM_lam,
                FLAGS.cSBM_p, FLAGS.cSBM_mu, FLAGS.cSBM_sigma,
            )
    name = "{sampler}_{model}-{hidden1}_{dataset}_NL-{NL:d}_BS-{BS:d}_E-{epochs:d}_eta-{eta:.2f}_delta-{delta:.2f}_MaxR-{maxR:.1f}_lr-{lr:.3f}_dropout-{dropout:.2f}_WD-{weight_decay:.5f}_Re-{repeat_test_sampling}_TInt-{test_interval}_{raw_name}"\
            .format(
...
def plot_policy(tb_logger, sampler, adj, train_nodes, labels_np, step, args, n_top=3, plot_label=None):
    labels_set = set(labels_np) if plot_label is None else [plot_label]
    for plot_label in labels_set:
        train_cluster = train_nodes[labels_np[train_nodes]==plot_label]
        count = 0
        ratios = []
        for sid in train_cluster:
            nei_nodes = np.array(adj[sid].rows[0])
            if nei_nodes.size <= args.neighbor_limit:
                continue
            nei_probs = np.array(sampler.get_sample_probs_list(sid))
            max_ind = np.argsort(nei_probs)[-n_top:]
            max_probs = nei_probs[max_ind]
            max_nei = nei_nodes[max_ind]
            label_count = (labels_np[max_nei] == plot_label).sum()
            ratios.append(label_count/n_top)
            count += 1
        ratio_np = np.array(ratios)
        tb_logger.log_value("Ratio_Mean_%d"%plot_label, np.mean(ratio_np), step=step)
        tb_logger.log_value("Ratio_Max_%d"%plot_label,  np.max(ratio_np), step=step)
        tb_logger.log_value("Ratio_Min_%d"%plot_label,  np.min(ratio_np), step=step)
        tb_logger.log_value("Ratio_Std_%d"%plot_label,  np.std(ratio_np), step=step)    
...
def main():
    ...
    adj_full, features, train_nodes, y_train, \
        valid_nodes, y_valid, test_nodes, y_test, labels_np = load_data(FLAGS.dataset, flag=FLAGS)
    if FLAGS.laplacian:
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for batch in iterate_minibatches(
            ...
            if FLAGS.plotreward:
                ...
                plot_policy(tb_logger, sampler, adj_full, train_nodes, labels_np, global_step, args=FLAGS, n_top=FLAGS.neighbor_limit, plot_label=None)
